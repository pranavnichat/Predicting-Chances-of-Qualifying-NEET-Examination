...
def store_responses_and_score(unique_id, responses, score, cursor):
    try:
        ...
        cursor.execute("INSERT INTO Results (UniqueID, mocktest2) VALUES (?, ?)", (unique_id, score))
...
# Main Streamlit app function for Mock Test 2
def app():
    ...
    try:
        ...
        # Check if Mock Test 2 has already been completed
        cursor.execute("SELECT * FROM UserMockTests WHERE UniqueID = ? AND MockTestNumber = ?", (unique_id, 2))
        row = cursor.fetchone()
        if row:
            st.warning("You have already completed Mock Test 2.")
            return
        ...
        if 'test_started' in st.session_state and st.session_state['test_started']:
            ...
            while datetime.now() - start_time < duration:
                ...
                # Submit button
                if st.button('Submit Mock Test 2'):
            ...
            st.write("Time's up for Mock Test 2! Thank you for taking the test.")

            if 'test_completed' in st.session_state and st.session_state['test_completed']:
                # Calculate score
                score = calculate_score(responses, questions)

                # Store the responses and score in the database for Mock Test 2
                store_responses_and_score(unique_id, responses, score, cursor)

                # Generate PDF report for Mock Test 2
                generate_pdf_report(questions, responses, f"{username}_Mock_Test_2_Report.pdf")

                # Update completion status for Mock Test 2
                cursor.execute("INSERT INTO UserMockTests (UniqueID, MockTestNumber, Completed) VALUES (?, ?, ?)", (unique_id, 2, 1))
                cursor.commit()

                # Show the score and option to view responded questions and answers
                st.write(f"Your score for Mock Test 2: {score}/{len(questions)}")
