...
def store_responses_and_score(unique_id, responses, score, cursor):
    try:
        ...
        cursor.execute("INSERT INTO Results (UniqueID, mocktest3) VALUES (?, ?)", (unique_id, score))
        cursor.commit()  # Commit the transaction
    except pyodbc.Error as e:
        st.error(f"An error occurred while storing the responses and score in the database: {e}")

# Main Streamlit app function for Mock Test 3
def app():
    st.title("Mock Test 3 for NEET Examination")
    ...
    try:
        ...
        # Check if Mock Test 3 has already been completed
        cursor.execute("SELECT * FROM UserMockTests WHERE UniqueID = ? AND MockTestNumber = ?", (unique_id, 3))
        row = cursor.fetchone()
        if row:
            st.warning("You have already completed Mock Test 3.")
            return
        ...
        if 'test_started' in st.session_state and st.session_state['test_started']:
            ...
            while datetime.now() - start_time < duration:
                ...
                # Submit button
                if st.button('Submit Mock Test 3'):
            ...
            st.write("Time's up for Mock Test 3! Thank you for taking the test.")

            if 'test_completed' in st.session_state and st.session_state['test_completed']:
                # Calculate score
                score = calculate_score(responses, questions)

                # Store the responses and score in the database for Mock Test 3
                store_responses_and_score(unique_id, responses, score, cursor)

                # Update completion status for Mock Test 3
                cursor.execute("INSERT INTO UserMockTests (UniqueID, MockTestNumber, Completed) VALUES (?, ?, ?)", (unique_id, 3, 1))
                cursor.commit()

                # Show the score
                st.write(f"Your score for Mock Test 3: {score}/{len(questions)}")
